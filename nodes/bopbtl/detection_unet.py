# Vendored from microsoft/Bringing-Old-Photos-Back-to-Life (Global/detection_models/networks.py)
# Copyright (c) Microsoft Corporation. Licensed under the MIT License.
#
# Only the scratch-detection UNet is kept (the file's sync_bn wrapping is a
# no-op in the original code -- `self = DataParallelWithCallback(self)`
# reassigns a local variable inside __init__ and has no effect on the
# constructed instance -- so it and its Synchronized-BatchNorm dependency
# are dropped here). Weights load from the official FT_Epoch_latest.pt
# checkpoint (see Global/detection.py in the upstream repo) without change.
...
class UNet(nn.Module):
    """U-Net (Ronneberger et al., 2015), used by BOPBTL to predict a
    scratch/damage probability mask from a grayscale input image."""
