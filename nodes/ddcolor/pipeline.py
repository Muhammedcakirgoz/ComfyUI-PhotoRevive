# Vendored from piddnad/DDColor (ddcolor/pipeline.py)
...
class ColorizationPipeline:
    """Shared image colorization pipeline.
    ...
    def process(self, img_bgr: np.ndarray) -> np.ndarray:
        ...
        with ctx():
            ...
            tensor_gray_rgb = (
                torch.from_numpy(img_gray_rgb.transpose((2, 0, 1))).float().unsqueeze(0).to(self.device)
            )
